...
\input{GUI}
\input{zvuk}
\chapter{Software}  % TODO uvod - cemu rikam software a cemu firmware
\input{PyAlarmClock}
\input{HomeAssistant}
...
\begin{acronym}[EEPROM] % nejdelsi zkratka kvuli zarovnani
    \acro{CLI}{příkazový řádek\acroextra{ (command line interface)}}
    \acro{EEPROM}{elektricky vymazatelná paměť pouze pro čtení\acroextra{ (electrically erasable programmable read-only memory)}}
    \acro{IDE}{vývojové prostředí\acroextra{ (integrated development environment)}}
    \acro{ISR}{obsluha přerušení\acroextra{ (interrupt service routine)}}
    \acro{LED}{elektroluminiscenční dioda\acroextra{ (light-emitting diode)}}
    \acro{LSB}{nejméně významný bit\acroextra{ (least significant bit)}}
    \acro{LUT}{vyhledávací tabulka\acroextra{ (lookup table)}}
    \acro{MCU}{mikrokontrolér\acroextra{ (jednočipový počítač; microcontroller unit)}}
    \acro{MSB}{nejvýznamnější bit\acroextra{ (most significant bit)}}
    \acro{SDK}{software development kit}
    \acro{SMD}{součástka pro povrchovou montáž\acroextra{ (surface mount device)}}
    \acro{SRAM}{statická polovodičová pamět s~přímým přístupem\acroextra{ (static random access memory)}}
\end{acronym}
