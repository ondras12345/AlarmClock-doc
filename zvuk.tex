...
\begin{itemize}[nosep]
    \item frekvence $f$ -- $\frac{f - \SI{32}{\hertz}}{\SI{32}{\hertz}}$,
    \item amplituda -- \num{0} je ticho, \num{255} je plná hlasitost,
    \item délka trvání $t > 0$ -- $\frac{t}{\SI{25}{\milli\second}}$.
\end{itemize}
...
provádějící zápis jednoho bytu dat na určitou adresu.
Pro zjednodušení použití těchto příkazů byl vytvořen program
\shellcmd{EEPROM-tool.py} (umístěný ve složce \repopath{examples/} v repozitáři
\gitrepo{PyAlarmClock}).
% TODO eew, eer, EEPROM-tool.py
...
Proto je výhodné využít editor binárních dat GNU \shellcmd{poke}. % TODO footnote
...
s nimi pracujeme jako s objekty.
% TODO GNU poke
